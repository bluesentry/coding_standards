...
| **Files/Modules**      | lowercase, use underscores or hyphens (e.g. `networking.tf`, `variables.tf`)                           | lowercase or kebab-case for filenames; PascalCase for React component files (matching component name) | lowercase, underscores allowed (e.g. `data_processor.py`); Python packages lowercase (no spaces) |
